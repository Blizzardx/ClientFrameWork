...
public class Drawing
{
    ...
    public static Texture2D PaintLineInMask(Vector2 from, Vector2 to, float rad, Texture2D tex, Texture2D mask, out int pixcelCount)
    {
        pixcelCount = 0;
        ...
        for (int y = 0; y < (int)lengthY; y++)
        {
            for (int x = 0; x < (int)lengthX; x++)
            {
                ...
                if (pixels[y * (int)lengthX + x].a == 0 && maskPixels[y * lengthX + x].a != 0)
                {
