...
public class UIInput : MonoBehaviour
{
	...
	public string Validate (string val)
	{
		...
		//修改UIInput的显示字符串逻辑，如果超出字符串长度，不显示后面的字符串 [5/27/2015 Edison]
		//新加对汉字的逻辑处理 [5/27/2015 Edison]
		if(characterLimit > 0)
		{
			int nValidLen = 0;
			int i = 0;
	...
	// Edison 增加对中文的字符长度判断
	int GetLengthWithChinese(StringBuilder sb)
	{
		int result = 0;
	...
	// Edison 增加对中文的字符长度判断
	int GetLengthWithChinese(string s)
	{
		int result = 0;
		for (int i = 0; i < s.Length; ++i)
		{
...
//		int rl = right.Length;
		int rl_chinese = GetLengthWithChinese(right);   // Edison 增加对中文的字符长度判断
...
//			if (characterLimit > 0 && sb.Length + rl >= characterLimit) break;
			if (characterLimit > 0 && GetLengthWithChinese(sb) + rl_chinese >= characterLimit) break;   // Edison 增加对中文的字符长度判断
